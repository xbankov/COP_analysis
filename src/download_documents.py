...
def scrape_url(url, current_data_csv, current_html):
    if not current_data_csv.exists() or config.FORCE["PARSE"]:
        scraper = DocumentScraper(url, current_data_csv, current_html)
...
def main():
    logger.info("##########################################")
    logger.info("############ DOWNLOAD DOCUMENTS  #########")
    logger.info("##########################################")
    ...
    for name, url in config.DECISION_URLS.items():
        ...
        # DOWNLOAD
        start_time = time.time()
        download_files(data, data_path, data_folder, "DocumentName")
        end_time = time.time()
        logger.info(f"Downloading files finished in: {end_time - start_time} seconds")

        # EXTRACT TEXT
        start_time = time.time()
        extract_pdfs(data, data_path, data_folder, "DocumentName")
        end_time = time.time()
        logger.info(
